...
use crate::core::models::{NoteMeta, FileMeta};
use crate::error::{CodexError, CodexResult};

use super::encryption::EncryptionManager;
use super::storage::{save_note_file, load_note_file, delete_note_file, save_vault_file, load_vault_file, delete_vault_file};
...
impl Vault {
    ...
    pub async fn save_file(&self, filename: String, data: Vec<u8>) -> CodexResult<String> {
        let id = Uuid::new_v4().to_string();
        let key = get_key()?;

        let encrypted = EncryptionManager::encrypt(key, &data)?;
        save_vault_file(&id, &encrypted)?;

        let now = Utc::now().timestamp();
        let size = data.len() as i64;

        sqlx::query(
            "INSERT INTO files (id, filename, size, created_at, updated_at) VALUES (?, ?, ?, ?, ?)"
        )
        .bind(&id)
        .bind(&filename)
        .bind(size)
    ...
    pub async fn list_files(&self) -> CodexResult<Vec<FileMeta>> {
        let rows = sqlx::query(
            "SELECT id, filename, size, created_at, updated_at FROM files ORDER BY updated_at DESC"
        )
        .fetch_all(&self.db)
        .await?;
    ...
    pub fn load_file(&self, id: String) -> CodexResult<Vec<u8>> {
        let key = get_key()?;

        let encrypted = load_vault_file(&id)?;
        let decrypted = EncryptionManager::decrypt(key, &encrypted)?;

        Ok(decrypted)
    }

    pub async fn get_file_meta(&self, id: String) -> CodexResult<FileMeta> {
        let row = sqlx::query(
            "SELECT id, filename, size, created_at, updated_at FROM files WHERE id = ?"
        )
        .bind(&id)
        .fetch_one(&self.db)
        .await?;
    ...
    pub async fn delete_file(&self, id: String) -> CodexResult<()> {
        // delete encrypted file
