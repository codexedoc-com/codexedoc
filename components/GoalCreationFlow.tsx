...
export function GoalCreationFlow({ onClose, onGoalCreated, userId }: GoalCreationFlowProps) {
  ...
  return (
    <div className="w-full max-w-2xl bg-white border border-zinc-200/80 rounded-2xl sm:rounded-3xl shadow-sm p-4 sm:p-8 md:p-10">
      ...
      <div className="mb-5 sm:mb-6">
        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-indigo-50 border border-indigo-100 text-[11px] sm:text-xs font-medium text-indigo-700 mb-2 sm:mb-3">
          ...
          {step === "title" && (
            ...
            >
              <input
                ...
                className="w-full rounded-xl border border-zinc-300 px-3.5 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base text-zinc-900 placeholder:text-zinc-400 focus:border-indigo-600 focus:ring-2 focus:ring-indigo-100 focus:outline-none transition"
                autoFocus
              />

              <div>
                <p className="text-[11px] sm:text-xs font-medium text-zinc-500 mb-2">Popular suggestions:</p>
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {PRESET_GOALS.map((preset) => (
                    <button
                      ...
                      className="text-xs px-2.5 sm:px-3 py-1.5 rounded-lg border border-zinc-200 bg-zinc-50 text-zinc-700 hover:bg-zinc-100 hover:border-zinc-300 transition cursor-pointer"
                    >
                      {preset}
                    </button>
                  ))}
            ...
            >
              {whyOptions.map((option) => (
                <button
                  ...
                  className={`p-3.5 sm:p-4 rounded-xl border text-left transition cursor-pointer ${
                    formData.why === option.label
                      ? "border-indigo-600 bg-indigo-50/50 ring-1 ring-indigo-600"
                      : "border-zinc-200 bg-white hover:border-zinc-300 hover:bg-zinc-50"
                  }`}
                >
                  <p className="font-semibold text-xs sm:text-sm text-zinc-900">{option.label}</p>
                  <p className="text-[11px] sm:text-xs text-zinc-500 mt-0.5 sm:mt-1">{option.desc}</p>
                </button>
              ))}
            </motion.div>
          )}
      ...
      <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-zinc-100 flex items-center justify-between">
        <button
          ...
          className="flex items-center gap-1.5 text-xs sm:text-sm font-medium text-zinc-500 hover:text-zinc-900 transition cursor-pointer py-1"
        >
        ...
        {step !== "time" ? (
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl bg-indigo-600 px-4 sm:px-5 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 disabled:opacity-40 transition cursor-pointer active:scale-[0.99]"
          >
            Next
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl bg-indigo-600 px-4 sm:px-6 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 disabled:opacity-40 transition cursor-pointer active:scale-[0.99]"
          >
            {pending ? "Creating..." : "Finish & Open"}
